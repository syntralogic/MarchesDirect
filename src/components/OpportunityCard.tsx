    ...
    <div className="brand-card brand-border border rounded-xl p-4 hover:border-orange/40 transition-all duration-200 group">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <button
          ...
          aria-label={saved ? t('favoriteRemove') : t('favoriteSave')}
        >
          {saved ? (
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        >
          {saved ? (
            <BookmarkCheck size={14} className="text-orange" />
          ) : (
      ...
      <div className="flex items-center justify-between">
        <span className={`text-xs px-2 py-0.5 rounded-full border ${getStatusColor(opportunity.status)}`}>
          {opportunity.status}
        </span>
