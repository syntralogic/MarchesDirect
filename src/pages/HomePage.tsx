...
// ---------------------------------------------------------------------------
// HERO — compacted so the three path cards + Démo/Témoignages fit on mobile
// ---------------------------------------------------------------------------
function HeroSection({ onAppt, onCallback }: { onAppt: () => void; onCallback: () => void }) {
  const [demoOpen, setDemoOpen] = useState(false);
  return (
    <section className="px-3 md:px-6 pt-2 md:pt-10 pb-0 md:pb-10 max-w-3xl mx-auto w-full">
      <div className="border border-orange/40 rounded-2xl bg-[#061D32] p-2.5 md:p-6 orange-glow relative overflow-hidden">
        <div className="relative z-10">
          <span className="text-[9px] md:text-[11px] font-bold text-orange uppercase tracking-widest">
            Artisans · TPE · PME
          </span>
          <h1 className="text-lg md:text-3xl font-extrabold leading-tight mt-1 md:mt-2 mb-1.5 md:mb-3">
            <span className="text-white">Trouvez des marchés adaptés</span>{' '}
            <span className="text-orange">à votre entreprise.</span>
          </h1>
          <div className="mb-2 md:mb-4"><OpportunityPaths onDemoClick={() => setDemoOpen(true)} /></div>
          <div className="flex flex-col sm:flex-row gap-1.5 md:gap-3">
            <button onClick={onAppt} className="flex-1 bg-orange text-white font-semibold py-2 md:py-3 rounded-xl text-xs md:text-sm hover:bg-orange/90 transition-colors">
              Prendre rendez-vous
            </button>
            <button onClick={onCallback} className="flex-1 border border-orange text-orange font-semibold py-2 md:py-3 rounded-xl text-xs md:text-sm hover:bg-orange/10 transition-colors">
              Être rappelé
            </button>
          </div>
        </div>
  ...
  return (
    <div className="grid grid-cols-1 gap-1 md:gap-2">
      {paths.map(p => {
        ...
        return (
          <Link key={p.href} to={p.href} className="flex items-center gap-2 md:gap-3 bg-[#061D32]/80 border border-[#17334D] rounded-xl p-1.5 md:p-3 hover:border-orange/50 group transition-all">
            <div className="w-8 h-8 md:w-11 md:h-11 rounded-lg bg-orange/10 flex items-center justify-center shrink-0">
              <p.icon size={16} className="text-orange md:hidden" />
              <p.icon size={22} className="text-orange hidden md:block" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-xs md:text-sm font-semibold text-white group-hover:text-orange transition-colors">{p.title}</div>
              <div className="text-[10px] md:text-[11px] text-[#B9BBC8] mt-0.5 leading-snug hidden sm:block md:block">{p.sub}</div>
            </div>
            <ChevronRight size={14} className="text-orange shrink-0 md:hidden" />
      ...
      <div className="grid grid-cols-2 gap-1 md:gap-2">
        <button onClick={onDemoClick} className="flex flex-row sm:flex-col items-center sm:items-start gap-1.5 sm:gap-1 md:gap-2 bg-[#061D32]/80 border border-[#17334D] rounded-xl p-1.5 md:p-3 hover:border-orange/50 group transition-all text-left">
          <PlayCircle size={16} className="text-orange md:hidden shrink-0" />
          <PlayCircle size={22} className="text-orange hidden md:block" />
          <div className="min-w-0">
            <div className="text-[11px] md:text-sm font-semibold text-white group-hover:text-orange transition-colors truncate">Démo vidéo</div>
            <div className="text-[10px] md:text-[11px] text-[#B9BBC8] leading-snug hidden md:block">Le parcours en 1 min</div>
          </div>
        </button>
        <a href="#mdh-temoignages" className="flex flex-row sm:flex-col items-center sm:items-start gap-1.5 sm:gap-1 md:gap-2 bg-[#061D32]/80 border border-[#17334D] rounded-xl p-1.5 md:p-3 hover:border-orange/50 group transition-all">
          <PlayCircle size={16} className="text-orange md:hidden shrink-0" />
          <PlayCircle size={22} className="text-orange hidden md:block" />
          <div className="min-w-0">
            <div className="text-[11px] md:text-sm font-semibold text-white group-hover:text-orange transition-colors truncate">Témoignages vidéo</div>
            <div className="text-[10px] md:text-[11px] text-[#B9BBC8] leading-snug hidden md:block">Leurs retours d'expérience</div>
          </div>
        </a>
      </div>
    </div>
