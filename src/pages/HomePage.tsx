...
import map from "@/assets/map.png"; // Added map image import
...
function MapController() {
  ...
  return (
    <div className="absolute top-3 right-3 flex flex-col gap-2 z-[1000]">
      <button onClick={() => map.zoomIn()} className="w-8 h-8 bg-[#061D32] border border-[#17334D] rounded-lg text-white hover:bg-orange/20 transition-colors text-lg font-bold">+</button>
      <button onClick={() => map.zoomOut()} className="w-8 h-8 bg-[#061D32] border border-[#17334D] rounded-lg text-white hover:bg-orange/20 transition-colors text-lg font-bold">-</button>
    </div>
  );
}
    ...
    <section className="px-4 md:px-6 py-6 md:py-10 max-w-7xl mx-auto w-full">
      ...
      <div className="border border-[#17334D] rounded-2xl bg-[#061D32] p-4 md:p-6 hover:border-orange/30 transition-all">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* MAP WITH OVERLAY BEHIND IMAGE AND MARKERS */}
          <div className="relative h-[250px] md:h-[350px] rounded-xl overflow-hidden border border-[#17334D]">
            ...
            {/* Map Image Layer - On top of the map but below markers */}
            <div 
...
function OpportunityPaths() {
  const paths = [
    { icon: Building, title: 'Marchés publics', sub: 'Mairies, État, collectivités', href: '/marches-publics' },
