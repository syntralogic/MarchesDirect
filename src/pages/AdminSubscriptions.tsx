...
export default function AdminSubscriptions() {
  ...
  const load = () => {
    ...
    adminApi.subscriptions({ status: status === 'all' ? undefined : status, q: query || undefined, limit: 50 })
      ...
      .catch(err => setError(getApiErrorMessage(err, t('adminLoadError') || 'Impossible de charger les abonnements.')))
  ...
  const handleCancel = async (sub: ApiAdminSubscription) => {
    if (!window.confirm(t('adminSubscriptionsCancelConfirm', { company: sub.company_name }) || `Annuler l'abonnement de ${sub.company_name} à la fin de la période en cours ?`)) return;
    ...
    try {
      ...
      showToast(t('adminSubscriptionsCancelSuccess'));
    } catch (err) {
      showToast(getApiErrorMessage(err, t('adminSubscriptionsCancelFailed')), 'error');
    } finally {
      setCancelingId(null);
    }
  };
  ...
  return (
    <AdminLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-white">{t('adminSubscriptionsTitle')}</h1>
        <p className="text-sm text-[#B9BBC8]">{t('adminSubscriptionsSub')}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {STATUS_TABS.map(s => (
          ...
          >
            {s === 'all' ? t('all') : s} {s === 'all' ? `(${totalCount})` : statusCounts[s] ? `(${statusCounts[s]})` : ''}
          </button>
        ))}
      </div>
      ...
      <form onSubmit={e => { e.preventDefault(); load(); }} className="flex gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder={t('adminSubscriptionsSearch')} className="w-full bg-[#061D32] border border-[#17334D] rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder:text-[#B9BBC8] focus:outline-none focus:border-orange" />
        </div>
        <button type="submit" className="flex items-center gap-2 bg-[#061D32] border border-[#17334D] text-[#B9BBC8] px-4 py-2.5 rounded-xl hover:text-white transition-colors text-sm">
          <Search size={16} /> {t('adminSubscriptionsSearchButton')}
        </button>
      </form>
      ...
      {loading ? (
        ...
        <div className="bg-[#061D32] border border-[#17334D] rounded-xl p-10 text-center"><p className="text-sm text-[#B9BBC8]">{t('adminSubscriptionsEmpty')}</p></div>
      ) : (
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    {sub.cancel_at_period_end && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full text-orange bg-orange/10">{t('adminSubscriptionsCancelled')}</span>}
                  </div>
                  ...
                  <p className="text-xs text-[#B9BBC8]">
                    ...
                    {sub.current_period_end && ` · ${t('adminSubscriptionsRenewal')} ${new Date(sub.current_period_end).toLocaleDateString('fr-FR')}`}
                  </p>
                </div>
