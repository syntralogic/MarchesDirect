...
export default function MarchesPublicsPage() {
  ...
  const FilterPanel = () => (
    <div className="space-y-5">
      ...
      <div>
        <label className="text-xs font-semibold text-[#B9BBC8] uppercase tracking-wide mb-2 block">Statut</label>
        <select value={status} onChange={e => setStatus(e.target.value)} className="w-full bg-[#061D32] border border-[#17334D] rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-orange appearance-none">
  ...
  return (
    <div className="page-fade-in max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-10">
      ...
      <div className="mb-6 md:mb-8">
        <span className="text-xs font-bold text-orange uppercase tracking-widest">BOAMP · PLACE · JOUE</span>
        <h1 className="text-2xl md:text-3xl font-extrabold text-white mt-1 mb-2">Marchés publics</h1>
        <p className="text-[#B9BBC8] text-sm md:text-base max-w-2xl">
          Mairies, État, collectivités. Conformité, pièces du DCE et mémoire technique générés pour vous.
        </p>
      </div>
      ...
      <div className="md:hidden flex gap-2 mb-4">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-colors ${hasFilters ? 'border-orange text-orange bg-orange/10' : 'border-[#17334D] text-[#B9BBC8]'}`}
        >
          <SlidersHorizontal size={14} />
          Filtres {hasFilters && `(${[location, sector !== 'Tous', status !== 'Tous', category !== 'Toutes'].filter(Boolean).length})`}
        </button>
      </div>

      {/* Result count mobile */}
      <div className="md:hidden mb-4">
        <span className="text-sm text-[#B9BBC8]"><span className="text-white font-semibold">{filtered.length}</span> opportunité{filtered.length !== 1 ? 's' : ''} trouvée{filtered.length !== 1 ? 's' : ''}</span>
      </div>
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="hidden md:flex items-center justify-between mb-4">
            <span className="text-sm text-[#B9BBC8]"><span className="text-white font-semibold">{filtered.length}</span> opportunité{filtered.length !== 1 ? 's' : ''} trouvée{filtered.length !== 1 ? 's' : ''}</span>
            <select className="bg-[#061D32] border border-[#17334D] rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-orange appearance-none">
      ...
      {filtersOpen && (
        <div className="fixed inset-0 z-50 flex items-end">
          ...
          <div className="relative w-full bg-[#031B30] border-t border-[#17334D] rounded-t-2xl p-5 max-h-[85dvh] overflow-y-auto z-10">
            ...
            <FilterPanel />
            <button onClick={() => setFiltersOpen(false)} className="w-full mt-5 bg-orange text-white font-semibold py-3 rounded-xl">
