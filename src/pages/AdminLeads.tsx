...
export default function AdminLeads() {
  ...
  const load = () => {
    ...
    adminApi.opportunityLeads({ status: status === 'all' ? undefined : status, limit: 100 })
      ...
      .catch(err => setError(getApiErrorMessage(err, t('adminLoadError') || 'Impossible de charger les demandes.')))
  ...
  const handleGrant = async (lead: ApiAdminOpportunityLead) => {
    ...
    try {
      ...
      showToast(t('adminLeadsGrantSuccess') || 'Accès complet accordé');
    } catch (err) {
      showToast(getApiErrorMessage(err, t('adminLeadsGrantFailed') || "Échec de l'octroi d'accès."), 'error');
    } finally {
      setGrantingId(null);
    }
  };
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      {isLevel3 && <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full text-green-400 bg-green-400/10"><ShieldCheck size={10} /> {t('adminLeadsFullAccess') || 'Accès complet'}</span>}
                    </div>
                  ...
                  <div className="flex flex-col gap-2 shrink-0">
                    ...
                    >
                      ...
                      {openJourneyId === lead.id ? t('adminLeadsHideJourney') : t('adminLeadsShowJourney')}
                    </button>
                    <button
                    ...
                    >
                      ...
                      {isLevel3 ? t('adminLeadsGranted') : t('adminLeadsGrant')}
                    </button>
                  </div>
                </div>
                ...
                {openJourneyId === lead.id && (
                  <div className="mt-4 pt-4 border-t border-[#17334D]">
                    {journeyLoading === lead.id ? (
                      <div className="flex items-center gap-2 text-xs text-[#B9BBC8]"><Loader2 size={13} className="animate-spin" /> {t('adminLeadsJourneyLoading')}</div>
                    ) : !journeys[lead.id] || journeys[lead.id].length === 0 ? (
                      <p className="text-xs text-[#B9BBC8]">{t('adminLeadsNoJourney')}</p>
                    ) : (
